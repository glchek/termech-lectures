...
\[\sum_{k=1}^{N} \left( \bar F_k - \underbracket{\bar w_k^{\text{кор}} m_k}_{J^{\text{кор}}} - \underbracket{\bar w_k^{\text{пер}} m_k}_{J_k^{\text{пер}}} - \bar w_k^{\text{отн}} m_k \right) \delta \bar r_k = 0\]
любая связь может быть заменена её реакцией
...
$\bar v_k = 0 \quad \bar w_k = 0$ и общее уравнение динамики, тогда
\[\boxed{\sum_{k=1}^{N} \bar F_k \delta r_k = 0} \qquad \text{Принцип виртуальных перемещений}\]
Система нафходится в равновесии $\iff$ работа активных сил на виртуальных перемещениях равна нулю
...
\[\sum_{k=1}^{N} \bar F_k \delta r_k = \sum_{i=1}^{n} Q_i \delta q_i =0 \iff Q_i = 0\]
$\delta \bar r_k$ зависимы, а $\delta q_i$ независимы
\[
\begin{cases}
\delta q_k \neq 0 \\
\delta q_i = 0 \; i \neq k
