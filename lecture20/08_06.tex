...
\begin{enumerate}
    ...
    \end{enumerate}
    \item 
    Регулярная прецессиия динамически симметричного($A = B$) твёрдого тела
    \[\begin{cases}
        \dot p A + q r( C - A) = 0, \\
        \dot q B + p r ( A - C) = 0, \\
        \dot r C = 0
    \end{cases} \qquad \dot r \implies r = r_0 = \text{ const}\]
    \[C r_0 = C r = K_{O_z} = K_o \cos \theta = \text{ const} \qquad K_O = \frac{C r_o}{\cos \theta} \qquad \theta = \theta_0 = \arccos \frac{C r_0}{K_O} = \text{ const}\]
    \[A p = K_{O_x} = K_O \sin \theta \sin \varphi \implies p = \frac{K_O}{A}\sin \theta \sin \varphi\]
    \[A q = K_{O_y} = K_O \sin \theta \cos \varphi \implies q = \frac{K_O}{A} \sin \theta \cos \varphi\]
    \begin{figure}[H]
    \centering
    ...
    \boxed{
    \begin{cases}
    \theta_0 = \text{ const} \qquad \cos \theta_0 = \frac{C}{K_O} \cdot r_0  \\
    \dot \psi = \frac{K_O}{A} \\ 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{precession.jpg}
    \caption{}
    \label{fig:myimage} 
    \end{figure}
\begin{enumerate}
...
\end{proof}
    ...
    \end{proof}
    \item По построению $OP \| \bar \omega$ \[OP = \lambda \bar \omega \qquad \lambda = \frac{1}{\sqrt{2 T}}\]
    \begin{proof}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{aks.jpg}
    \includegraphics[width=0.45\textwidth]{web.jpg} %fixme будто съехало
    \caption{}
    \label{fig:myimage} 
    \end{figure}
