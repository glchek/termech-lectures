...
\begin{proof}
     \[\qquad E = A A^T \qquad O = \dot A A^T + A \dot A^T \qquad \dot A A^T = -(\dot A A^T)^T \qquad \Omega = - \Omega^T\] %fixme оформить красиво столбиками
\end{proof}
    \[
    \begin{aligned}
...
\[\bar v_Q' = [\bar \omega, \bar r'_Q]\]
\[\bar r_a' = \bar r_Q - \bar r_P = \overline{PQ}\]
\[\bar v_Q' = \bar v_Q - \bar v_P\]
Тогда
\[\bar v_Q - \bar v_P = \bar \omega \times (\bar r_Q - \bar r_P)\] %fixme исправить векторные произведения на скобки(или не исправлять), надо что-то придумать
