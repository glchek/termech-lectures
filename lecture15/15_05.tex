...
\begin{document}
\lecture{}{15.05}
\[\frac{d}{d t} \frac{\partial T}{\partial \dot q} - \frac{\partial T}{\partial q_i} = Q_i = - \frac{\partial \Pi}{\partial q_i} + Q_i^*\]
\[\frac{d}{d t} \frac{\partial L}{\partial \dot q} - \frac{\partial L}{\partial q_i} = Q_i^*\]
\begin{theorem}
    Уравнения Лагранжа разрешимы относительно обобщённых ускорений
\end{theorem}
\begin{proof}
    \[
    \begin{aligned}
    ...
    %fixme по-хорошему записать ровно слева T, а справа коэффициенты
    \[T = T_2 + T_1 + T_0\]
    \[T_2 = \frac{1}{2} \sum_{i, j = 1}^{n} a_{i j} \dot q_i \dot q_j, \qquad a_{i j} = \sum_{k=1}^{N} m_k \frac{\partial \bar r_k}{\partial q_i} \frac{\partial \bar r_k}{\partial q_j}\]
    \[T_1 = \sum_{i=1}^{n} a_i \dot q_i \qquad a_i = \sum_{k=1}^{N} m_k \frac{\partial \bar r_k}{\partial q_i} \frac{\partial \bar r_k}{\partial t}\]
    \[T_0 = \frac{1}{2} \sum_{k=1}^{N} m_k \left(\frac{\partial \bar r_k}{\partial t}\right)^2\]
    Существует теорема Эйлера об однородных функций(из которой следует вид производных этих многочленов)
    \[\frac{d}{d t} \frac{\partial (T_2 + T_1 + T_0)}{\partial \dot q} - \frac{\partial (T_2 + T_1 + T_0)}{\partial q_i} = Q_i \qquad \frac{\partial T_0}{\partial \dot q_i} = 0 \qquad \frac{\partial T_1}{\partial \dot q_i} = a_i \qquad \frac{\partial T_2}{\partial \dot q_i} = \sum_{j=1}^{n} a_{i j } \dot q_j\]
    ...
    \textbf{Основаная теорема Лагранжевого формализма}
    ...
    Так как потенциальная энергия не зависит от скоростей и ускорений:
...
\begin{theorem}[Об изменении полной энергии голономной системы]
    \[
    \begin{aligned}
    \frac{d}{d t} T &= \frac{d}{d t}\sum_{i=1}^{n} \frac{\partial T}{\partial \dot q_i} \dot q_i - \sum_{i=1}^{n} \frac{d}{d t}\left(\frac{\partial T}{\partial q_i} \right)\dot q_i + \sum_{i=1}^{n}  \frac{\partial T}{\partial q_i} \dot q_i +\frac{\partial T}{\partial t}  \\
    &= \frac{d}{d t} \left( \sum_{i=1}^{n} \frac{\partial T}{\partial \dot q_i} \dot q_i\right)  - \sum_{i=1}^{n}\left(\frac{d}{d t} \frac{\partial T}{\partial \dot q_i} - \frac{\partial T}{\partial q_i} \right) \dot q_i + \frac{\partial T}{\partial t} = \frac{d}{d t} (2 T_2 + T_1) - \sum_{i=1}^{n} Q_i^* \dot q_i + \sum_{i=1}^{n} \frac{\partial \Pi}{\partial q_i} \dot q_i+ \frac{\partial T}{\partial t} \\
    &= 2 \frac{d T}{d t} - \frac{d}{d t} (T_1 + 2 T_0) - \sum_{i=1}^{n} Q_i^* \dot q_i  + \frac{\partial \Pi}{\partial t} - \frac{\partial \Pi}{\partial t} + \frac{\partial T}{\partial t}
\end{aligned}
    \]
    \[\underbracket{\frac{d T}{d t} + \frac{d \Pi}{d t}}_{\frac{d E}{d t}} = \sum_{i=1}^{n} Q_i^* \dot q_i + \frac{\partial \Pi}{\partial t} + \frac{d}{d t} (T_1 + 2 T_0) - \frac{\partial T}{\partial T}\]

\end{theorem}
Мощность непотенциальных сил, частная производная непотенциальной энергии и вклад нестационарных связей

В консервативных систмах:
\[\frac{d E}{d t} = 0\]
\end{document}