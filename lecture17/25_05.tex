...
\begin{proof}
    ...
    \[
        \begin{aligned}
            Q_i = \frac{-\partial \Pi}{\partial q_i} + Q_i^* \qquad Q_i(q, \dot q) &\approx Q_i(0, 0) + \sum_{j=1}^{n} \left. \frac{\partial Q_i}{\partial q_j} \right|_{0} q_j + \sum_{j=1}^{n}  \left. \frac{\partial Q_i}{\partial \dot q_j}\right|_{0} \dot q_j \\
            ...
            &= - \sum_{j=1}^{n} c_{i j} q_j - \sum_{j=1}^{n} c_{i j}^* q_{i j} - \sum_{j=1}^{n} b_{ i j} \dot q_j 
        \end{aligned}
        \]
        
        $c_{i j}$ — квази-упругие коэффициенты
        \[\frac{d}{d t} \sum_{j=1}^{n} a_{i j}^0 \dot q_j - 0 = \sum_{j=1}^{n} a_{i j}^0 \ddot q_j = Q_i\]
