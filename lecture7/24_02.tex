...
\begin{table}[H]
    ...
    \begin{tabular}{c|c|c}
        & Геом & Дифф \\
        Стац & $f(\bar r_1, \dots , \bar r_N) = 0$ & $f(\bar r_1, \dots , \bar r_N, \bar v_1, \dots , \bar v_N) = 0 \qquad \sum^N_{k=1} \bar a_k (\bar r_1, \dots, \bar r_N) \bar v_k + a_0 (\bar r_1, \dots ,\bar r_N) = 0$ \\ % примеры диф. сввяей, которые встречаются(линейные по скоростям)
