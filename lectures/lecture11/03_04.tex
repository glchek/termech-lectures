...
\begin{proof}
    Докажем, что существует $P^*: \bar r$:
     \[\bar M_{P^*} = \bar M_P + \bar R \times \left( \bar r - \bar r_P\right) = \alpha \bar R \qquad (\bar M_P , \bar R) = \alpha R^2 \qquad \alpha = \frac{I_2}{I_1}\]
     \[\bar M_P + \bar R \times \left( \bar r - \bar r_P\right) = \frac{I_2}{I_1} \bar R \qquad \bar R \times \left( \bar r - \bar r_P \right) = \begin{pmatrix}
        R_x \\ R_y \\ R_z
...
\subsection{Изменение количества движения}
Дифференциальная форма
\[\dot{\bar Q} = \sum_{k=1}^{N} m_k w_k = \sum_{k=1}^N F_k = \bar R\]
Интегральная форма:
\[\Delta \bar Q = \bar Q_2 - \bar Q_1 = \int_{t_1}^{t_2} \bar R dt \quad \text{ — импульс силы }\]
Следствие: Если $\bar R = 0$, то центр масс либо покоится, либо движется равномерно прямолинейно
\begin{proof}
    $\dot {\bar Q} = M \bar w_C$
\end{proof}


\subsection{Изменение момента количества движения}
Дифференциальная форма
\[\dot {\bar K}_P  = \bar Q \times \bar v_P + \bar M_P\]
Интегральная форма
\[\Delta \bar K_O = \int_{t_1}^{t_2} \bar M_O dt \text{ — импульс момента}\]
где $O$ — неподвижная точка
\begin{proof}
    \[
        \begin{aligned}
            \dot{\bar K}_P &= \frac{d}{d t}\left(\sum_{k=1}^{N} (\bar r_k - \bar r_P) \times m_k \bar v_k \right)
            = \sum_{k=1}^N \bar v_k \times m_k \bar v_k - \sum_{k=1}^N \bar v_P \times m_k \bar v_k  + \underbracket{\sum_{k=1}^{N} (\bar r_k - \bar r_P) \times \underbracket{m_k \bar w_k}_{\bar F_k}}_{\bar M_P}
            \\ &= \bar Q \times v_P + \bar M_P
        \end{aligned}
        \]
\end{proof}
Следствия:        
        \[\bar v_P = 0 \iff \dot {\bar K}_P = \bar M_P  \qquad \bar v_P = \bar v_C \iff \dot{\bar K}_C = \bar M_C\]
        \begin{proof}
...
Дифференциальная форма
\[\dot T = \sum_{k=1}^N(\bar F_k, \bar v_k)\]
Интегральная форма
\[T_2 - T_1 = \int_{t_1}^{t_2} d' A = A_{1 2}\]
\begin{proof}
\[\dot T = \frac{1}{2} \frac{d}{d t}\left(\sum_{k=1}^N m_k v_k^2\right) = \sum_{k=1}^N m_k (\dot{\bar v}_k, \bar v_k)  = \sum_{k=1}^N (\bar F_k, \bar v_k)\]
это совокупная мощность всех сил, то есть мощность внутренних и внешних сил

\[d T = \frac{1}{2} \sum_{k=1}^N m_k d v_k^2 = \sum_{k=1}^N (m_k \bar w_k d t, \bar v_k) = \sum_{k=1}^N (\bar F_k , d \bar r_k) = d' A\]
