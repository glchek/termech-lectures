...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        $A \neq B \; \&  \; B \neq C  \; \& \; A \neq C \implies$ хотя бы одна пара из проекций угловой скорсти равна нулю,
        то есть вращение вокруг координатных осей
    \end{enumerate}
    \item 
    ...
    \begin{dcases}
        ...
        B q = K_{O_{y'}} = K_O \sin \theta \cos \varphi,  \\
    ...
    \implies
     \begin{dcases} 
        ...
        q = \frac{K_O}{B} \sin \theta \cos \varphi,\\
    ...
    \[
    \begin{dcases}
    p = \dot \psi \sin \theta_0 \sin \varphi = \frac{K_O}{A} \sin \theta_0 \sin \varphi \\
     q = \dot \psi \sin \theta_0 \cos \varphi = \frac{K_O}{B} \sin \theta_0 \cos \varphi \\ r = \dot \psi \cos \theta_0 + \dot \varphi
...
\begin{figure}[H]
    ...
    \label{fig:myimage} 
    \end{figure}
\[: \pi \text{ — кас к эллипс в т. } P \qquad \overline{OP} = \lambda \bar \omega\]
\begin{enumerate}
    \item $\pi \perp \bar K_O$
    \begin{proof}
...
\end{proof}
    ...
    \begin{proof}
        \[OQ = \frac{(\bar OP, \bar K_O)}{K_O} = \frac{A x p + B y q + C z r}{K_O} = \frac{\lambda (A p^2 + B q^2 + C r^2)}{K_O} = \frac{\frac{1}{\sqrt{2 T}} 2 T}{K_O} = \frac{\sqrt{2 T}}{K_O}\]
    \end{proof}
    \item 
...
\noindent \keypoint Замечание: как бы аккуратно не раскручивали тело, вокруг $z$ вращаться не будет, так как седло — положение неусточивое, то есть если твёрдое тело раскрутить вокруг оси соответствующей среднему моменту инерции, то угловая скорость начнёт периодически менять своё направление (эффект Джанибекова)
