...
\noindent \keypoint \[
\bar Q' = M \bar v_c ' = 0 \qquad \bar K_p' = \bar K_C' = \bar K_C\]
\begin{proof}
    \[\begin{aligned}
...
\end{pmatrix}
\]
Такие площади $S_x, S_y, S_z$ — называются площадями миделей
\section{Тензор инерции}
\Def Момент инерции относительно оси $\bar u$:
...
&= \sum_{k=1}^{N} (x_k^2 ( \beta^2 + \gamma^2)+ y_k^2 (\alpha^2 + \beta^2) + z_k^2 (\alpha^2 + \beta^2 )  - 2 \alpha \beta x_k y_k - 2 \alpha \gamma x_k z_k - 2\beta \gamma y_k z_k) m_k \\
&= \alpha^2 \sum_{k=1}^{N} m_k(y_k^2 + z_k^2) + \beta^2 \sum_{k=1}^{N} m_k (x_k^2 + z_k^2) + \gamma^2 \sum_{k=1}^{N} m_k(y_k^2 + x_k^2) + \\
&- 2 \alpha \beta \sum_{k=1}^{N} m_k x_k y_k - 2 \beta \gamma\sum_{k=1}^{N} y_k z_k - 2 \alpha \gamma \sum_{k=1}^{N} m_k x_k z_k \\
&= \alpha^2 J_x + \beta^2 J_y + \gamma^2 J_z - 2 \alpha \beta J_{x y} - 2 \beta \gamma J_{y z} - 2 \alpha \gamma J_{x z}  = \bar u^T J \bar u
\end{aligned}
\] % fixme
Тензор инерции:
\[J = \begin{pmatrix}
