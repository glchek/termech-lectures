...
\begin{proof}
    ...
    \end{aligned}
    \]
    \[\sum_{k=1}^{N} m_k (z_k z_C + y_k y_C) = \sum_{k=1}^{N} m_k z_k z_C + \sum_{k=1}^{N} m_k y_k y_C = M z_C^2 + M y_C^2 = M(z_C^2 + y_C^2) \]
    \[
    \begin{aligned}
    ...
    \]
\end{proof}
$\bar u_C$ — ось проходящая через центр масса, сонаправленная с $\bar u$
\[J_{\bar u} = J_{\bar u_C} + M d^2 \qquad d \text{ — расстояние между осями}\]
%fixme картинка с двумя осями, и расстоянием между ними
\[J_{\bar u} = \bar u ^T J \bar u = \alpha^2 J_x + \beta^2 J_y + \gamma^2 -2 \alpha \beta J_{xy} - 2 \beta \gamma J_{yc} - 2\alpha \gamma J_{xz}\]
\keypoint Положительно определённая квадратичная форма % fixme почему?

Тогда существует ск, в которой она задаёт эллипс
\[A x^2 + B y^2 + C z^2 = 1 \]
\Def главная система координат — система координат в которой тензор инерци твёрдого тела имеет вид 
\[J = \begin{pmatrix}
    A & 0& 0\\0 & B & 0\\ 0&0& C 
\end{pmatrix}\]
Её оси — главными осями твёрдого тела
\Def Эллипсоид инерции
\[A x^2 + B y^2 + C z^2 = 1 \]
...
\[
\begin{aligned}
\bar K &= \sum_{k=1}^{N} \bar r_k \times \bar v_k m_k = \sum_{k=1}^{N} \bar r_k \times (\bar \omega \times \bar r_k) m_k = \sum_{k=1}^{N} (\bar \omega r_k^2 - \bar r_k (\bar \omega, \bar r_k)) m_k = \sum_{k=1}^{N} \omega (\bar u r_k^2 - \bar r_k (\bar u, \bar r_k)) m_k  \\
&= \sum_{k=1}^{N} \omega \bar u \left( r_k^2 - (\bar u, \bar r_k)^2 \right) m_k = \bar \omega J_{\bar u} \text{ муть какая-то} \\
...
\[T = \frac{1}{2} \sum_{k=1}^{N} m_k v_k^2 = \frac{1}{2} \sum_{k=1}^{N } m_k (\omega \times \bar r_k)^2 = \frac{\omega^2}{2}  \underbracket{ \sum_{k=1}^{N} m_K(\bar u \times \bar r_k)^2}_{J_{\bar u}}  =  \frac{\omega^2 J_{\bar u}}{2} = \frac{1}{2}\bar \omega^T J \bar \omega = \frac{1}{2} \bar \omega^T \bar K_O = \frac{1}{2} (\bar \omega , \bar K_O)\]
