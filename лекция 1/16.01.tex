...
\newcommand{\Def}{%
    \text{Def}% Написание букв
    \hspace{-1.2em}% Сдвиг назад, чтобы черта оказалась поверх букв
    \raisebox{0.1ex}{/}% Сама черта (можно использовать / или \diagup)
    \hspace{0.5em}% Пробел после значка
}

% Вариант через наложение (более аккуратный)
\newcommand{\sDef}{%
    \mathrel{\rlap{/}\text{Def}} 
}
...
\begin{enumerate}
    ...
    \item Материальная точка — геометрическая точка, которой в соответсвии поставлен скларя, называемый массой
    $$(\bar r, m)$$
    \item Каждой паре математический точек может быть поставлена в соответсвие пара векторов $\vec F_1, \vec F_2$, которые называются силами, причём
$$\bar F_1 = - \bar F_2$$
    \item В Евклидовом пространстве можно найти такую систему коородинат и такой способ параметризации $t$, что 
\[m \ddot r = \bar F\]
\end{enumerate}

\subsection{Основные понятия кинематики:}
\begin{enumerate}
    \item \[\bar r = \{x,y,z\} = \{x,y,z\}^T\] %вектор столбец/ транспонированный
...
\begin{enumerate} %FIXME Есть картинка
    \item Траектория
    \item $S(t)$
\end{enumerate}
...
% есть запись, надо переписать FIXME
\begin{figure}[H]
    \centering
...
\[\bar v = \frac{d \bar r}{dt} = \frac{d \bar r}{dt} \cdot \frac{ds}{dt} = \bar \tau v \implies \bar \tau = \frac{\bar v}{v}\]
...
\begin{figure}[H]
    ...
    \label{fig:myimage} % Sets a label for cross-referencing
\end{figure}
\[K := \lim_{\Delta s \to 0} \frac{\Delta \phi}{\Delta s} = \lim_{\Delta s \to 0} \frac{|\Delta \tau|}{|\bar \tau| \Delta s} = |\lim_{\Delta s \to 0} \frac{\Delta \bar \tau}{\Delta s} = |\frac{d \bar \tau}{ds}|\] %FIXME
\[1 = ( \bar \tau(s), \bar \tau(s)),\]
\[
0 = (\frac{d \bar \tau}{ds}, \bar \tau) + ( \bar \tau , \frac{d \bar \tau}{ds}) = 2(\frac{d \bar \tau}{ds}, \bar \tau) 
\]
\Def $\;$ Вектор главной нормали:
\[\bar n = \frac{d \bar \tau}{d s} / |\frac{d \tau}{ds}| = \frac{1}{K} \frac{d\bar \tau}{d s}\]
...
\Def \;
$k v^2 := w_n \; \; \; \;\dot v := v_\tau$ 

\[\rho := \frac{1}{K}\]
\[w_n= kv^2 = \frac{v^2}{\rho}\]

\paragraph{Касательные окружности совпадают,} когда разложение в ряд Тейлора совпадает до 2ой степени
\end{document}