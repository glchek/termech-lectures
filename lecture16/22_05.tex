...
\begin{proof}
    ...
    \begin{enumerate}
        \item 
        $T = 0 \iff \dot q = 0 \quad T \ge 0$, у $\Pi$ в нуле строгий минимум, тогда $\exists \dot O_\Delta : p \in \dot O_\Delta \implies E(p) > 0$
        \item
        ...
        если траектория начинает движение из $O_\delta(O)$, то она никогда не покинет $O_\varepsilon$, так как на границе $O_\varepsilon$ энергия не меньше $E^*$, а $E^{**} < E^*$
    \end{enumerate}
