...
\begin{enumerate}
    ...
    $I_2 = (\bar v_P, \bar \omega) = \text{пр}_{\bar \omega} \bar v_p \cdot \omega $
    \item 
...
\[\bar r = \bar r_{O'} + A r' \implies r' = A^{-1} (\bar r - \bar r_{O'})\]
...
\[\dot A r' = \dot A A^T (\bar r - \bar r_{O'}) = \Omega (\bar r - \bar r_{O'}) = \bar \omega \times (\bar r - \bar r_{O'}) \]
Из структуры формул понятно, что $\bar v_{O'} + \bar \omega \times (\bar r - \bar r_{O'})$ — переносная скорость, $A \dot {\bar r}'$ — относительная скорость
\end{proof}
...
&\left| \dot A \dot{\bar r}' = \Omega \bar v^{\text{отн}} = \bar \omega \times v^{\text{отн}}\right| \\
&= \bar w^{\text{пер}} + \underbracket{2(\bar \omega \times v^{\text{отн}})}_{\bar w^{\text{Кор}}} + \bar w^{\text{отн}} = \bar w^{\text{пер}} + \bar w^{\text{кор}} + \bar w^{\text{отн}} 
 \end{aligned}
 \]
 Из структуры формул понятно, что обозначения введены корректно
\end{proof}
\section{Сложное движение}
\keypoint
для $A = A_2 A_1 = A_1 A_2'$, $\bar r = A \bar r_0 = A_1 A_2' \bar r_0$
\[\bar v = \Omega \bar r = \bar \omega \times \bar r \qquad \bar \omega = \bar \omega_1 + \bar \omega_2\]
\begin{proof}\[
\begin{aligned}
...
&= \Omega_1 \bar r + A_1 \Omega_2' A_2' \bar r_0 = \Omega_1 \bar r + \Omega_2 \bar r = \bar \omega_1 \times \bar r + \bar \omega_2 \times \bar r
\end{aligned}
\]
\end{proof}
\noindent \keypoint \[\boxed{\bar \varepsilon = \bar \varepsilon_1 + \bar \varepsilon_2 + \bar \omega_1 \times \bar \omega_2}\]
\begin{proof}
    \[\bar \varepsilon = \frac{d}{dt} \bar \omega = \dot{\bar \omega} = \frac{d}{dt}{\left(\bar \omega_1 + \bar \omega_2\right)} = \frac{d}{dt} (\bar \omega_1 + A_1 \bar \omega_2') = \bar \varepsilon_1 + \dot A_1 \bar \omega_2' + A_1 \dot {\bar \omega}_2'\]
    \[A_1 \dot{\bar {\omega}}_2' = A_1 \bar \varepsilon_2' = \bar \varepsilon_2  \qquad \dot A_1 \bar \omega_2' = \dot A_1 A_1^T \bar \omega_2 = \bar \omega_1 \times \bar \omega_2\]
\end{proof}
\end{document}