...
\begin{proof}
\[\overline{q_1 \circ  q_2} = \overline{(a_1 + \vec a_1) \circ (a_2 + \vec a_2)} = \overline{a_1 a_2 - (\vec a_1, \vec a_2) + a_1 \vec a_2 + a_2 \vec a_1 + \vec a_1 \times \vec a_2} = a_1 a_1 - (a_1, a_2) - a_1 \vec a_2 - a_2 \vec a_1 - \vec a_1 \times \vec a_2\]
\[\overline{(a_2 + \vec a_2)\circ (a_1 + \vec a_1)} = (a_2 - a\vec a_2) \circ (a_1 - \vec a_1) = a_! a_@ - (\vec a_1,\vec a_2 ) - a_1 \vec a_2 - a_2 \vec a_1 + \vec a_2 \times \vec a_1\]
\end{proof}
...
\end{aligned}\]
\[\psi_\Lambda(\vec a_1) \circ \psi_\Lambda(\vec a_2) = - \underbracket{(\psi_\Lambda(\vec a_1), \psi_\Lambda(\vec a_2))} + (\psi_\Lambda(\vec a_1) \times \psi_\Lambda(\vec a_2)) = - \underbracket{(a_1, a_2)} + \psi_\Lambda(\vec a_1 \times \vec a_2) \]
